...
acc_prop = AccurateWignerPropagator(**sys_params)
acc_ground_state = acc_prop.get_ground_state(abs_tol_purity=1e-10).get().real
acc_propagated_state = acc_prop.propagate(100000).get().real

fast_prop = FastWignerPropagator(**sys_params)
fast_ground_state = fast_prop.get_ground_state(abs_tol_purity=1e-10).get()
fast_propagated_state = fast_prop.propagate(100000).get()
...
img_params = dict(
    ...
    aspect=1,
    cmap='bwr',
    norm=WignerNormalize(vmin=-5e-8, vmax=5e-8),
)
